...
public class MusicDAO {
	...
	private final String URL="jdbc:oracle:thin:@localhost:1521:XE";
		...
		// 기능
		// 순위, state, idcrement, title, singer, album
		public ArrayList<MusicVO> musicListData(int page){
			...
			try{
				...
				System.out.println("musicListData"+ex.getMessage());
			}
			finally
			{
				disConnection();
		...
		public int musicTotalPage(){
			...
			{
				System.out.println("musicTotal"+ex.getMessage());
			}
			finally
			{
				disConnection();
		...
		/*
		 	...
		 	INSERT
		 	 ...
		 	 = INSERT INTO table(값1...) => DEFAULT와 관련없이 무조건 첨부
		...
		public MusicVO musicDetailData(int no)
		{
			...
			try{
				...
				ps.executeUpdate();
				/*
				 		ps.executeUpdate(); => 오라클의 데이터가 변경(INSERT,DELTE,UPDATE)
				 			=> COMMITl
				 		 ps.executeQuert(); => 데이터의 변경이 없는 상태 (검사개:SELECT)
				 */
		...
		public ArrayList<MusicReplyVO> replyListData(int mno){
			ArrayList<MusicReplyVO> list=new ArrayList<>();
			try{
				getConnection();

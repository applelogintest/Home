...
public class FoodDetailServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out=response.getWriter(); // 브라우저가 읽어갈 위치 지정 PrintWirter는 auto flush()여서 따로 버퍼에있는 데이터들을 지우라고 명령안해줘도 비워준다
		...
		for(FoodHouseVO fvo:list)
		{
			...
			out.println("<img src="+fvo.getImage().substring(0,fvo.getImage().indexOf("^"))+" width=80>");
			out.println("</td>");
